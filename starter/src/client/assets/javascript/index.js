  ...
  return new Promise((resolve) => {
	...
	*/
    const race = setInterval(async () => {
      const raceRes = await getRace(raceID);
      console.log(raceRes);
      if (raceRes.status === "finished") {
        renderAt("#race", resultsView(raceRes.positions));
        resolve();
        return clearInterval(race);
      }

      renderAt("#leaderBoard", raceProgress(raceRes.positions));
    }, 500);
    /* 
